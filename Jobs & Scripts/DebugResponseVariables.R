  ...
  if (all(!is.na(dat))) {
    rvs <- purrr::map(wind, env = parent.frame(), function(.x, env){
      ...
      ras <- zoo::rollapplyr(dat[,c(cl_nm, sa_nm)], width = .x, align = "left", by.column = F, env = parent.frame(), na.pad = T, partial = T, function(r, env){
        ...
        p <- tryCatch({quantmod::findPeaks(r[, 1], thresh = sd(r[,1]))}, error = function(e) numeric(0))
        v <- tryCatch({quantmod::findValleys(r[, 1], thresh = sd(r[,1]))}, error = function(e) numeric(0))
      ...
      while (n < end) {
        ...
        p <- tryCatch({c(n:end)[quantmod::findPeaks(dat[n:end, cl_nm], thresh = sd(dat[n:end, cl_nm]))]}, error = function(e) numeric(0))
        v <- tryCatch({c(n:end)[quantmod::findValleys(dat[n:end, cl_nm], thresh = sd(dat[n:end, cl_nm]))]}, error = function(e) numeric(0))

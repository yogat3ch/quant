...
TSL <- function(v, tsla = NULL, tslp = NULL, retro = c(n = NULL, hilop = NULL), verbose = F) {
  ...
  out <- purrr::map(1:nrow(v), .f = function(.x, env = parent.frame()) {
    i <- day <- .x # Save as the increment i and initial index day
    tsl <- 0
    price <- as.numeric(v[i, cl_cl, drop = T])
    peak <- as.numeric(v[i, cl_cl, drop = T])
    # Set initial values
    while (price >= peak - tsl) {
      if (all(!is.null(retro))) {  
        ...
        tsl <- {as.numeric(max(v[tpi:i, hi_cl, drop = T])) - as.numeric(min(v[tpi:i, lo_cl, drop = T]))} * retro[[2]] # TSL is set at the proportion of the range of hi/los over a given retrograde period
      } else if (!is.null(tsla) & is.numeric(tsla)) tsl <- tsla else tsl <- tslp * peak
      #if (verbose) print(c(.x = .x, tsl = tsl,price = price, peak = peak, i = i))
      price <- as.numeric(v[i, lo_cl, drop = T])
      peak <- as.numeric(max(v[day:i, hi_cl, drop = T]))
      if (i == nrow(v)) break
      i <- i + 1
    }
    #if (verbose) print(c(peak = peak, price = price, GL = price - v[day, cl_cl]))
    out <- (as.numeric(price) - as.numeric(v[day, cl_cl, drop = T])) / as.numeric(v[day, cl_cl, drop = T]) #Get the gain or loss as proportion and add it to out vector
    return(c(out, i - day))
  })

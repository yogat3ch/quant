...
#' @param .data \code{[xts, data.frame, tibble_time]} Data to be used **Required**
...
#' @param out.vector \code{[logical]} Return a vector of the buy/sell positions as a factor 1 for buy, -1 for sell, the rest 0.
#' @param with.gains \code{[logical]} Return a vector of the corresponding percent gains with each purchase.
...
# For debugging:
# list(.data = Positions_ts[[1]], percent = .05, returns.clm = "tslp_0.18_rv", tslindex.clm = NULL, .opts = list(bs.v = F, with.gains = F, max.gain = F)) %>% list2env(envir = .GlobalEnv)
# test <- optimReturn(.data = Positions_ts[[1]], returns.clm = "tslp_0.18_rv", percent = .05, .opts = list(bs.v = T, with.gains = T, max.gain = T))
 #filter_all(.vars_predicate = dplyr::all_vars({. != 0})) %>% View
optimReturn <- function(.data, percent = 0.05, returns.clm = NULL, tslindex.clm = NULL, .opts = list(bs.v = F, with.gains = F, max.gain = F)) {
  tslindex.clm <- ifelse(!is.null(tslindex.clm),tslindex.clm,paste0(returns.clm,"_ind"))
  td_nm <- stringr::str_extract(names(.data), "^time$|^date$") %>% subset(subset = !is.na(.)) %>% .[1]
  v <- .data[, c(td_nm, returns.clm, tslindex.clm)] %>% dplyr::mutate_at(dplyr::vars(!!tslindex.clm), dplyr::funs(as.POSIXct, .args = list(origin = lubridate::origin)))
  lgl.v <- .data[, returns.clm, drop = T] > percent
  # if there are no positive values at all
  if (sum(lgl.v) == 0) {
    return(NULL)
  }
  rle.df <- HDA::rleIndex(rle(lgl.v))
  v.ind <- .data[, tslindex.clm, drop = T]
  cl_nm <- c(high = stringr::str_extract(colnames(.data), stringr::regex("^high", ignore_case = T)) %>% subset(subset = !is.na(.)),
             low =  stringr::str_extract(colnames(.data), stringr::regex("^low", ignore_case = F)) %>% subset(subset = !is.na(.)),
  ...
  out.res <- purrr::map(1:20, pf = sys.frame(sys.nframe()), .opts = .opts, function(.x, pf, .opts){
    ...
    total.value <- init.inv <- as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T]) # the amt of initial investment
    ...
    if (.opts$max.gain) {
      ...
      max.gain[i] <- (max(pf$.data[i:i_sell, cl_nm[c("high")], drop = T] %>% unlist) - pf$.data[i, cl_nm[c("close")], drop = T]) / pf$.data[i, cl_nm[c("close")], drop = T]
    }
    # Calculate running values
    total.value <- as.numeric(pf$.data[i_sell, pf$cl_nm["close"], drop = T]) * shares # as total position value
    rem <- total.value %% as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T])
    total.returns <- (as.numeric(pf$.data[i_sell, pf$cl_nm["close"], drop = T]) - as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T])) / init.inv + total.returns # total returns as numeric
    ...
    while (i <= length(pf$lgl.v)) {
      if (pf$lgl.v[i]) { # if the immediate next value is a positive gain
        ...
        if (total.value %/% as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T]) < 1 ) break # Break if the value drops below being able to buy a share
        shares <- {total.value %/% as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T])} #Update shares
        rem <- total.value %% as.numeric(pf$.data[i, pf$cl_nm["close"], drop = T]) # calculate the remainder left when purchasing additional shares
        # Calculate running values
        total.value <- as.numeric(pf$.data[i_sell, pf$cl_nm["close"], drop = T]) * shares + rem # as total position value

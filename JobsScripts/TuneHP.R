...
purrr::iwalk(dat_wf, ~{
  ...
  .best_wf <- workflowsets::rank_results(.tg, 
                                      rank_metric = metric, 
                                      select_best = TRUE)
  wf <- workflowsets::pull_workflow(wflow_set, .best_wf$wflow_id)
  .best_wf_res <- workflowsets::pull_workflow_set_result(.tg, .best_wf$wflow_id)
  ...
  if (any(purrr::map_int(.best_hp, length) > 1)) {
    ...
    .tg <- tune::tune_grid(
      wf,
      resamples = .d_roll,
      param_info = dials::parameters(qf::parameter_create(.best_hp, .tunes)),
      grid = 10,
      control = tune::control_grid(
        verbose = TRUE,
        allow_par = TRUE,
    ...
    .best_hp <- tune::show_best(.tg, metric = metric, n = 1) %>%
      dplyr::select(.tunes$name)
  } 
...
tictoc::toc(log = TRUE)
qf::iMessage(glue::glue("{lubridate::now()} - TuneHP Complete - Elapsed: {tic.log(format = T) %>% stringr::str_subset('overall')}"))
DBI::dbDisconnect(db)

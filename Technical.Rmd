...
```{r 'Add RVs to Data'}
dat <- purrr::map(Positions_ts_rv, ~ dplyr::select(.x, - dplyr::ends_with("rv"), - dplyr::ends_with("ind")))
...
Positions_ts_rv <- purrr::map(Positions_v, function(.x){
  ...
  if (dplyr::select(.rv_data, dplyr::ends_with("ind")) %>% purrr::map(~ is.numeric(.x)) %>% any) {
    .rv_data <- dplyr::mutate_at(.rv_data, dplyr::vars(dplyr::ends_with("ind")), ~ lubridate::as_datetime(., origin = lubridate::origin))
  }
  ...
  if (any(dplyr::select(.rv_data, dplyr::ends_with("ind")) %>% purrr::map(~ max(.x, na.rm = T)) > t_max)) {
    .rv_data <- dplyr::mutate_at(.rv_data, dplyr::vars(dplyr::ends_with("ind")), ~ {
      if (.x > t_max) .x <- NA_complex_ else .x
      .x
    })
  }
    attr(.rv_data, "Sym") <- .x
...
Positions_ts_rv_iv <- purrr::map2(Positions_ts_iv, Positions_ts_rv, function(.x, .y){
  s <- attr(.x, 'Sym')
  if (xts::is.xts(.x) & xts::is.xts(.y)) {
...
dat <- Positions_ts_rv_iv[11:20] # Pass the list of timeseries objects for which algorithms will trained
train_rvs <- NULL
best_tsl <- best_tsl[names(dat)]
modelSpecs <- modelSpecs[11:20]

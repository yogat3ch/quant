...
Positions_ts %<>% lapply(function(l){ # Actual Time for measuring exact weeks, quarters and approx. months
...
Positions_ts %<>% lapply(wind = c(wind,200) , function(l, wind){
  if (nrow(l) > max(wind) & all(!is.na(l[,c("high","low","close")])) ){
    out_sma <- purrr::map(.x = wind, l=l, function(.x, l){
...
Positions_ts %<>% lapply(wind = wind, function(l, wind){
  if (nrow(l) > max(wind) & all(!is.na(l[,c("high","low","close")])) ){
    adx <- purrr::map(.x = wind, l=l, function(.x, l){
...
Positions_ts %<>% lapply(wind = c(7, 7*4, 7*4*3), verbose = F, function(l, wind, verbose){
  if (nrow(l) > max(wind) & all(!is.na(l[,c("high","low","close")])) ) {
  WpR <- purrr::map(.x = wind, function(.x, env = parent.frame()){
  out <- TTR::WPR(l[,c("high","low","close")],n = .x)
    for (i in which(is.na(out))) {
      hmax <- max(l[1:i,"high"])
      if (verbose == T) print(hmax)
      lmin <- min(l[1:i,"low"])
      if (verbose == T) print(lmin)
      pctR <- ifelse({(hmax - l[i,"close"])/(hmax - lmin)} %>% is.nan, 0.5, (hmax - l[i,"close"])/(hmax - lmin))
      out[i] <- pctR
    }
  return(out)
  })
  if (verbose == T) print(WpR)
...
Positions_ts %<>% lapply(wind = wind, verbose = F, function(l, wind, verbose){
  if (nrow(l) > max(wind) & all(!is.na(l[,c("high","low","close")])) ){
    # ROC
  roc <- purrr::map(.x = wind, env = parent.frame(),function(.x, env){
  ...
  # momentum
  mom <- purrr::map(wind, env = parent.frame(), function(.x, env){
...
Positions_ts %<>% lapply(wind = sort(c(wind,14)), verbose = F, function(l, wind, verbose){
   if (nrow(l) > max(wind) & all(!is.na(quantmod::HLC(l))) ){
  atrs <- purrr::map(wind, env = parent.frame(), function(.x, env){
...
Positions_ts %<>% lapply(wind = wind, verbose = F, function(l, wind, verbose){
  ...
  colnames(out) <- paste0(names(out), ".", .x,"_rv")
  out <- cbind(l, out)
  }else out <- l
  return(out)
})
...
rstudioapi::jobRunScript(path = "~/R/Quant/RFAttrFilterandxgBoost.R", workingDir = "~/R/Quant/", importEnv = T,exportEnv = "R_GlobalEnv")
# HDA::startPkgs(c("tidyverse","dplyr","magrittr","caret"))
# test <- purrr::map(names(Positions_ts$GOOG)[grep("rv$", names(Positions_ts$GOOG))], dat = Positions_ts$GOOG, function(nm, dat){
#   iex <- names(dat)[grep("high|low|volume|open|close|change|changePercent|chg", names(dat))]
#   nms <- names(dat)[grep("rv$", names(dat))] %>% .[{. != nm}]
#   frm <- formula(paste(nm, "~ .", "-",paste0(nms, collapse = "-"),"-",paste0(iex, collapse = "-")))
#   dvs <- FSelector::random.forest.importance(frm, dat)
#   dvs %<>% rownames_to_column() %>% mutate(R = cume_dist(attr_importance)) %>% filter(R > .7) %>% .[["rowname"]]
#   frm <- formula(paste0(nm, "~",paste0(dvs, collapse = "+")))
#   fitControl <- trainControl(method = "repeatedcv", number = 5, repeats = 1, search = "grid")
#   model <- train(frm, data = dat, method = "xgbTree", trControl = fitControl)
#   return(model)
# })
names(test) <- names(Positions_ts$GOOG)[grep("rv$", names(Positions_ts$GOOG))]
mdl_performance <- rbindlist(purrr::map2(.x = test, .y = names(test), function(.x, .y){
purrr::pluck(.x,"results") %>% mutate(Model = .y)})
)
mdl_performance %>% filter(Rsquared > qnorm(.9, mean(mdl_performance$Rsquared), sd(mdl_performance$Rsquared)))
```

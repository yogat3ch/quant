...
gsPOS <- googlesheets::gs_url("https://docs.google.com/spreadsheets/d/1Iazn6lYRMhe-jdJ3P_VhLjG9M9vNWqV-riBmpvBBseg/edit#gid=0")
POS <- googlesheets::gs_read(gsPOS,ws=1)
# ----------------------- Tue Jun 26 13:52:59 2018 ------------------------#
# Import Dad's portfolio
Dad_positions2018.06.26 <- read.csv("~/R/Quant/Dad_positions2018-06-26.csv", stringsAsFactors=FALSE)
```
...
CostBasis18[, c("Proceeds","Cost.per.Share","Cost","Gain.Loss","Gain.Loss.Pct")] %<>% lapply(.,FUN=function(x)gsub("\\$|\\,|\\%","",x)) %>% lapply(as.numeric)
...
```{r 'Source IEX API R File'}
# download.file("https://raw.githubusercontent.com/imanuelcostigan/iex/master/R/iex.R",destfile="iex.R")
source("iex.R")
```
...
```{r 'Financial Stats'}
quotes <- iex_stock_req(tSymbols = POS[,1,drop=T],type = "quote", query_filter = c("marketCap","peRatio","week52High","week52Low","ytdChange"))
stats <- iex_stock_req(tSymbols = POS[,1,drop=T],type = "stats", query_filter = c("peRatioHigh","peRatioLow","week52change	","returnOnEquity","consensusEPS","ttmEPS","profitMargin","year5ChangePercent","year1ChangePercent","ytdChangePercent","month6ChangePercent","month3ChangePercent","month1ChangePercent","cash","debt"))
fin_stats <- sapply(names(quotes),function(n){c(quotes[[n]],stats[[n]])},simplify=FALSE)
```

```{r 'Add Risk Metrics'}
pos_scaled <- lapply(pos,FUN = function(l){l$close %<>% scale %>% as.numeric
  l})
key_stats <- lapply(seq_along(pos), pos=pos_scaled,timeframe = NULL, FUN = function(l,pos,timeframe){
if(!is.null(timeframe)){
  mod <- lm(close ~ seq_along(date), data = pos[[l]] %>% filter(date > lubridate::floor_date(lubridate::today(), unit = timeframe)))
}else {
  mod <- lm(close ~ seq_along(date), data=pos[[l]])
}
out <- vector("numeric",length = 4)
out[1] <- mod[["coefficients"]][["seq_along(date)"]]
...
names(out) <- c("coef","rmse","adj.r","pctNeg")
return(out)
})
names(key_stats) <- names(pos)

key_stats %<>% do.call("rbind",.) 
key_stats$symbol <- symbol
key_stats %<>% reshape2::melt()
names(key_stats) <- c("stat","timeframe","value","symbol")
key_stats %>% gather(key = "stat", value = "value")
```

```{r 'Display Financial Stats'}
fin_statsTEST <- do.call(rbind,fin_stats) %>% as.data.frame
DT::datatable()
```
...
positions <- iex_batch_req(tSymbols = POS[-c(1,2),1,drop=T][!is.na(POS[-c(1,2),1])], range = "1y", filter = iex_fields[c(1:3,6:8,10:11),1,drop=T])
dad_positions <- iex_batch_req(tSymbols = Dad_positions2018.06.26[-c(1),1,drop=T][!is.na(POS[-c(1,2),1])], range = "1y", filter = iex_fields[c(1:3,6:8,10:11),1,drop=T])
```

```{r 'Add Sign indicator_Change Date'}
# ----------------------- Tue Jun 26 14:54:21 2018 ------------------------#
# Add sign indiicator for color change in candlestick graph 

pos <- lapply(positions,FUN=function(l){l %<>% mutate(chg = sign(changePercent))
  l$chg %<>% as.factor
  l})
dad_pos <- lapply(dad_positions,FUN=function(l){l %<>% mutate(chg = sign(changePercent))
  l$chg %<>% as.factor
  l})
# ----------------------- Tue Jun 26 14:27:25 2018 ------------------------#
# Change date to a date
pos <- lapply(pos,FUN=function(l){l$date %<>% as.character %>% as.POSIXct(x = ., tz = "", format = c("%Y-%m-%d"))
  return(l)})
dad_pos <- lapply(dad_pos,FUN=function(l){l$date %<>% as.character %>% as.POSIXct(x = ., tz = "", format = c("%Y-%m-%d"))
  return(l)})
```
```{r 'Convert to TimeSeries xts'}
tpos <- lapply(pos,FUN=function(l){xts(l[,not(names(l) %in% "date")], order.by = l$date)})
dad_tpos <- lapply(dad_pos,FUN=function(l){xts(l[,not(names(l) %in% "date")], order.by = l$date)})
```
...
dad_pos_ggp <- lapply(seq_along(dad_pos),pos = dad_pos,FUN=function(l,pos){
  pos[[l]] %>% 

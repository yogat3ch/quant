...
```{r 'Add SMA'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add Simple Moving Averages") 
```
</div>
...
```{r 'Add ADX'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add ADX") 
```
...
```{r 'Add ADX Signal'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add ADX signal") 
```

```{r 'Add Compressed ADX'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add Compressed ADX") 
```
</div>
...
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add Williams Percent R") 
```
</div>
...
```{r 'Add RSI'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add RSI") 
```
...
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add RSI Indicator") 
```
</div>
...
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add ROC And Momentum") 
```
</div>
...
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add ATR")
```
</div>
...
# ----------------------- Sat May 05 10:15:47 2018 ------------------------#
# Average True Range could be useful for setting stop-loss
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add SAR")
```
...
```{r 'Add SAR Indicator'}
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add SAR Indicator")
```
```{r 'SAR Tune'}
...
knitr::read_chunk("Jobs & Scripts/AddAllVariablestoData.R", labels = "Add MACD Compressed")
```
</div>
...
```{r 'Save or Load Data'}
load(file = list.files() %>% .[stringr::str_which(., "gain")])
SL <- rstudioapi::document_position(443,1)
if (tibble::is_tibble(Positions_ts[[1]])) tsrange <- Positions_ts[[1]] %>% tibbletime::get_index_col() %>% range else Positions_ts[[1]] %>% time %>% range
save(Positions_ts, file = paste0("Positions_ts",paste(tsrange,collapse = "_"),".Rdata"))
load(file = list.files()[stringr::str_which(list.files(), "Positions_ts\\d")])
if (tibble::is_tibble(Positions_ts[[1]])) tsrange <- Positions_new[[1]] %>% tibbletime::get_index_col() %>% range else Positions_new[[1]] %>% time %>% range
save(Positions_new, file = paste0("Positions_new",paste(tsrange,collapse = "_"),".Rdata"))
load(file = list.files()[stringr::str_which(list.files(), "Positions_new")])
save(Positions_tsl, file = "Positions_tsl.Rdata")
load(file = "Positions_tsl.Rdata")
```
```{r, eval=F}
...
```{r 'Generic Add Signals'}
# First change Positions to object entitled dat
dat <- Positions_ts
# Set the signal windows desired
wind  <-  c(weeks = 7, moonphase = 7*2, mooncycle = 7*4, quarters = 7*4*3)
# Save the file
save(dat, wind, file = "dat.Rdata")
rstudioapi::jobRunScript("Jobs & Scripts/AddAllDVstoData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
# Once Completed return the variable to it's original name and save it
dat %>% assign("Positions_ts", . , envir = globalenv()) 
#Save
...
TSLvars <- list(
  ...
  cl_cl <- stringr::str_which(colnames(x),"close")
  sd(x[,cl_cl, drop = T]) * 2},
  retro = c(n = wind[1], hilop = .9),
  retro = c(n = wind[2], hilop = .9),
  retro = c(n = wind[3], hilop = .9),
  retro = c(n = wind[4], hilop = .9))
TSLvars %<>% append(seq(.04,.24,.02) %>% setNames(nm = rep("tslp",length(.))) %>% as.list)
add <- F

save(dat, wind, TSLvars, add, file = "dat.Rdata")
rstudioapi::jobRunScript("Jobs & Scripts/AddRVstoData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
# Once Completed return the variable to it's original name and save it
dat %>% assign("Positions_ts", . , envir = globalenv()) 
# Save
rstudioapi::setCursorPosition(SL)
...
rstudioapi::jobRunScript(path = "Jobs & Scripts/GainMixPeriod.R", workingDir = getwd(), importEnv = F,exportEnv = "R_GlobalEnv")
# Save
Positions_ts <- dat
rstudioapi::setCursorPosition(SL)
...
```{r 'Function to Optimize Returns'}
pct <- seq(.05, .25, .01)
names(pct) <- seq(.05, .25, .01)
dat <- Positions_ts
save(dat, pct, file = "dat.Rdata")
rstudioapi::jobRunScript(path = "Jobs & Scripts/MapReturns.R", workingDir = getwd(), importEnv = F,exportEnv = "R_GlobalEnv")
...
tsl_performance <- purrr::map_depth(Positions_ret, .depth = 3, .f = function(.x) {
  ...
  if (identical(d %>% arrange(desc(Returns)) %>% .[1,2],d %>% arrange(desc(Cum.Returns)) %>% .[1,2])){
  out <- d %>% arrange(desc(Returns)) %>% .[1,]
  } else {
      out <-  rbind.data.frame(d %>% arrange(desc(Returns)) %>% .[1,],d %>% arrange(desc(Cum.Returns)) %>% .[1,])
  }
  return(out)
 })
Positions_tsl <- tsl_performance
...
rstudioapi::jobRunScript(path = "Jobs & Scripts/AddBenchmarks.R", workingDir = getwd(), importEnv = F,exportEnv = "R_GlobalEnv")
```

```{r 'Test Ephemeris Correlation'}
Years <- read_csv("Years.csv")[c("Date", "SUN.SI", "MOON.SI", "MERCURY.SI","VENUS.SI", "MARS.SI")]
...
load(file = "Positions_tsl.Rdata")
methodList <- c("xgbTree", "nnet", "pcaNNet") # Add method names. See http://topepo.github.io/caret/available-models.html for details
...
dat <- Positions_ts # Pass the list of timeseries objects for which algorithms will trained
train_rvs <- c("Gain_type","Mix_type")
best_tsl <- Positions_tsl
save(methodList, dat, train_rvs, best_tsl, file = "dat_dt.Rdata")
# ----------------------- Mon Jun 03 06:12:02 2019 ------------------------#
# Important to filter all none profitable signals from the type prediction
rstudioapi::jobRunScript(path = "Jobs & Scripts/TrainModels.R", workingDir = getwd(), importEnv = F,exportEnv = "R_GlobalEnv")
...
```{r 'Gather new Data from Portfolio Sheet and add Signals'}
googlesheets::gs_auth(token = "~//R//sholsen_googlesheets_token.rds")
gsPositions <- googlesheets::gs_url("https://docs.google.com/spreadsheets/d/1Iazn6lYRMhe-jdJ3P_VhLjG9M9vNWqV-riBmpvBBseg/edit#gid=0")
...
Positions_new <-  purrr::map(.x = tickers, dat = Positions_hlc %>% filter(Date > {(time(Positions_ts_gain[[1]]) %>% max) - 200}), toNum = toNum, .f = function(.x, dat, toNum){
...
wind  <-  c(weeks = 7, moonphase = 7*2, mooncycle = 7*4, quarters = 7*4*3)
# Save the file
save(dat, wind, TSLvars, file = "dat.Rdata")
rstudioapi::jobRunScript("Jobs & Scripts/AddAllDVstoData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
Positions_new <- dat
...
save(dat, wind, TSLvars, file = "dat.Rdata") #TSLvars from Chunk 35: Add RVs to Data
rstudioapi::jobRunScript("Jobs & Scripts/AddRVstoData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
Positions_new <- dat
save("Positions_new", file = paste0("Positions_new.Rdata"))
```
...
dat <- Positions_new
best_tsl <- Positions_tsl
save(dat, best_tsl, OOTbegin, file = "dat.Rdata")
rstudioapi::jobRunScript("Jobs & Scripts/ApplyModelstoNewData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
Positions_strat <- dat
```

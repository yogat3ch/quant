...
ticker <- r$symbol
get_bars <- function(ticker, from = Sys.Date() - 6, to = Sys.Date(), timeframe = "minute", 
  limit = NULL) 
{
  ...
  headers = AlpacaforR::get_headers()
  if (!is.null(limit)) {
    if (limit > 1000) {
  ...
  ticker = ifelse(length(ticker) > 1, paste0(ticker, collapse = ","), 
    ticker)
  week_dates = AlpacaforR::get_calendar(from, to)$date
  ...
  bars = httr::GET(url = paste0(url, "/v1/bars/", timeframe, 
    ...
    "&end=", to), headers)
  bars = AlpacaforR::response_text_clean(bars)
...
Positions_ts <- get_bars(Positions_v, from = lubridate::today() - lubridate::years(4))
...
if (xts::is.xts(Positions_ts$GOOG)) {
   earliest.date <- min(time(Positions_ts$GOOG))
 } else {
   td_nm <- stringr::str_extract(names(Positions_ts$GOOG), "^time$|^date$") %>% subset(subset = !is.na(.)) %>% .[1]
   earliest.date <- min(Positions_ts$GOOG[[td_nm]])
 }
Positions_ext <- purrr::imap(Positions_ts, ed = earliest.date, wind = params$wind, function(.x, .y, ed, wind){
  ...
  if (xts::is.xts(.x)) {
   to <- min(time(.x)) - lubridate::days(1)
   from <- min(time(.x)) - {wind[[4]] * 4}
 } else {
...
xtsAttributes(out) <- list(Sym = .y)
    ...
    out <- data.frame(time = seq(min(.x[, td_nm, drop = T]), max(.x[, td_nm, drop = T]), by = "1 days"), stringsAsFactors = F) %>% left_join(.x) %>% na.locf 
  out <- tibbletime::tbl_time(out, index = !!td_nm)
  attr(out, "Sym") <- .y
}
return(out)
...
purrr::map(stringr::str_extract_all(names(Positions_ts[[1]]), ".*rv$") %>% subset(subset = nchar(.) > 0) %>% purrr::compact(), .data = Positions_ts[[1]], function(.x, .data){
  ...
  .data %>% ggplot(data = .,mapping = aes(x = time))+
geom_line(aes(y = y_var))+
  geom_line(aes(y = close)) + 
    xlab(.x)+
    ylab("")+
    ggtitle(paste0("Correlation: ",cor(.data[["y_var"]], .data[["close"]])))+
    theme(plot.title = element_text(hjust = .5),plot.subtitle = element_text(hjust = .5))
})
```
...
Positions_ts <- list.files(path = pdatapath)[stringr::str_which(list.files(path = pdatapath), paste0(datetime,"_rv\\.csv$"))] %>% setNames(object = ., stringr::str_extract(., "^[A-Z]+")) %>%  purrr::imap(function(.x, .y){
...
purrr::map2(.x = Positions_ts, .y = names(Positions_ts), function(.x, .y){
  print(.y)
  if (!xts::is.xts(.x)) {
  write_csv(.x, path = paste0("PositionData/",.y,min(.x$Time),"_",max(.x$Time),".csv"))
    } else {
    zoo::write.zoo(.x, file = paste0("PositionData/",.y,time(.x) %>% min,"_",time(.x) %>% max,".csv"), index.name = "Time", col.names = T)
  }
})
```
...
bymin <- list.files(path = "PositionData", pattern = "bymin", full.names = T) %>% purrr::imap(fT = function(time, Time) {
 ...
 out <- append(as.character(time[!is.na(time)]), as.character(Time[!is.na(Time)]))
  ...
  out <- read.csv(.x) %>% mutate(time = fT(time,Time)) %>% select(- Time)
...
wind  <-  list(weeks = lubridate::weeks(1), moonphase = lubridate::weeks(2), mooncycle = lubridate::weeks(4), quarters = lubridate::weeks(4*3))
# Save the file
...
dat %>% assign("Positions_ts_iv", . , envir = globalenv()) 
#Save
...
```{r 'Trim Positions_ext with IVs back to Positions_ts length'}
dat <- Positions_ext
# Set the signal windows desired
wind  <-  list(weeks = lubridate::weeks(1), moonphase = lubridate::weeks(2), mooncycle = lubridate::weeks(4), quarters = lubridate::weeks(4*3))
# Save the file
save(dat, wind, file = "dat.Rdata")
rstudioapi::jobRunScript("JobsScripts/AddIVstoData.R", workingDir = getwd(), exportEnv = "R_GlobalEnv", importEnv = F)
Positions_ext <- purrr::map2(Positions_ts, dat, function(.x, .y){
  s <- attr(.x, 'Sym')
  if (xts::is.xts(.x) & xts::is.xts(.y)) {
    out <- window(.y, start = min(time(.x)))
  } else {
     td_nm <- stringr::str_extract(names(.x), "^time$|^date$") %>% subset(subset = !is.na(.)) %>% .[1]
...
```
```{r 'Join with existing RVs'}
Positions_ts_rv_iv <- purrr::map2(Positions_ext, Positions_ts_rv, function(.x, .y){
  s <- attr(.x, 'Sym')
  if (xts::is.xts(.x) & xts::is.xts(.y)) {
...
# Once Completed return the variable to it's original name and save it
dat %>% assign("Positions_ts_rv", . , envir = globalenv()) 
# Save
rstudioapi::setCursorPosition(SL)
```
...
best.returns %>% group_by(sym, tsl) %>% filter(Cum.Returns == max(Cum.Returns))
#End best returns
...
Positions_tsl <- Positions_tsl %>% purrr::map(function(.x){names(.x)[c(1,2)] <- c("tsl","pct")
return(.x)
})
...
methodlist <- c("xgbTree", "xgbDART", "mlpWeightDecayML", "bagEarthGCV")
...
modelSpecs <- purrr::map(modelSpecs, tG = tuningGrids, mlist = methodlist, function(.x, tG, mlist){
  ...
  mS <- append(mS, purrr::map(mlist[!mlist %in% names(mS)], ~ caretEnsemble::caretModelSpec(method = ., tuneLength = 7))) %>% setNames(nm = c(names(mS),mlist[!mlist %in% names(mS)]))
  } 
...
nms <- names(Positions_ts_rv_iv)[!names(Positions_ts_rv_iv) %in% names(modelSpecs)]
names(nms) <- names(Positions_ts_rv_iv)[!names(Positions_ts_rv_iv) %in% names(modelSpecs)]
...
# Sort
modelSpecs <- modelSpecs[names(Positions_ts_rv_iv)]
```
...
dat <- Positions_ts_rv_iv[8:20] # Pass the list of timeseries objects for which algorithms will trained
train_rvs <- NULL
best_tsl <- best_tsl[names(dat)]
save(modelSpecs, dat, best_tsl, file = "dat_dt.Rdata")
...
GRPN %<>% mutate_at(vars(c(2:6)),funs(as.numeric))
...
BYND %<>% mutate_at(vars(c(2:6)),funs(as.numeric))
...
```{r 'Gather new Data from Portfolio Sheet'}
googlesheets::gs_auth(token = "~//R//sholsen_googlesheets_token.rds")
gsPositions <- googlesheets::gs_url("https://docs.google.com/spreadsheets/d/1Iazn6lYRMhe-jdJ3P_VhLjG9M9vNWqV-riBmpvBBseg/edit#gid=0")
...
Positions_new <- purrr::map(Positions_v, shts = shts, function(.x, shts){
  ...
  x %>% stringr::str_replace_all("\\$|\\,","") %>% as.numeric
    ...
    clm %<>% mutate_if(.pred = funs(is.factor),funs(toNum))

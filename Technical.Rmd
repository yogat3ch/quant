...
TS.adxsar %<>% lapply(FUN=function(l){
  l %<>% mutate(pr=PR(close,35,verbose=F)) 
})
...
winRV(rv="close",wind=c(7,49,196),dat=TS.adxsar$GOOG)
```
...
TS.adxsar %<>% lapply(per = c(7,35,140),FUN=function(l,per){
  WpR <- sapply(per,function(peri){
    TTR::WPR(l[,c("high","low","close")],n = peri)
  })
  colnames(WpR) <- paste("WpR",per,sep=".")
  cbind(l,WpR)
})
```
...
TS.adxsar %>% lapply(ns=c(dy=7,wk=35,mc=140,qu=420),FUN=function(l,ns){
  ...
  colnames(smas) <- paste("SMA",names(ns),sep=".")
  cbind(l,smas)
})
```

...
iex_stock <- function(symbols, type = "quote", filter = NULL){
  req <- sapply(symbols, function(s){
  url  <- paste(iex_endpoint(), s, type,sep = "/")
  })
  req <- sapply(req,FUN=function(r){
    httr::modify_url(r,query = paste("filter=",paste(filter,collapse=","),sep=""))
  })
  return(req)
}
iex_stock_req <- function(symbols, filter = NULL){
  reqs <- iex_stock(symbols, filter = filter)
...
if (httr::http_type(resp) != "application/json") {
    stop(paste("API did not return JSON. Error:", httr::content(resp)), call. = FALSE)
  }
  parsed <- jsonlite::fromJSON(httr::content(resp, "text"),simplifyVector = T,flatten = T)
  Sys.sleep(0.2)
  return(parsed)})
  names(resps) <- nms
  return(resps)
...
iex_batch <- function(symbols, range, filter = NULL) {
  q <- paste("symbols=",paste(symbols,collapse=","),paste("&types=chart",sep=""),paste("&range=",range,sep=""),sep="")
  if(!is.null(filter)){
...
iex_batch_req <- function(symbols, range, filter = NULL){
resp <- httr::GET(iex_batch(symbols, range, filter = filter))
if (httr::http_type(resp) != "application/json") {
...
positions <- iex_batch_req(symbols = POS[-c(1,2),1,drop=T][!is.na(POS[-c(1,2),1])], range = "1y", filter = iex_fields[c(1:3,6:8,10:11),1,drop=T])
dad_positions <- iex_batch_req(symbols = Dad_positions2018.06.26[-c(1),1,drop=T][!is.na(POS[-c(1,2),1])], range = "1y", filter = iex_fields[c(1:3,6:8,10:11),1,drop=T])
```
## Financials
```{r 'Financial Quote'}
quotes <- iex_stock_req(symbols = POS[-c(1,2),1,drop=T][!is.na(POS[-c(1,2),1])], filter = c("marketCap","peRatio","week52High","week52Low","ytdChange"))
```
